...
\r\
        ...
        root.Position = UDim2.new(1, -12, 0.5, 0) -- dockeado al borde derecho (draggable), al lado de la UI\r\
        root.BackgroundColor3 = Color3.fromRGB(20, 20, 24); root.BorderSizePixel = 0; root.Parent = gui\r\
...
\r\
        ...
        vf.BackgroundColor3 = Color3.fromRGB(6, 10, 6); vf.BorderSizePixel = 0\r\
        vf.Ambient = Color3.fromRGB(170, 170, 175); vf.LightColor = Color3.fromRGB(255, 255, 255)\r\
...
\r\
        -- box overlay (borde ESP representativo, sobre el 3D)\r\
        ...
        box.Size = UDim2.fromOffset(66, 150); box.ZIndex = 3; box.Parent = vf\r\
...
\r\
        -- matrix rain (columnas verdes low-alpha delante del modelo)\r\
        self._matrix = {}\r\
        for i = 1, 10 do\r\
            local l = Instance.new(\"TextLabel\"); l.BackgroundTransparency = 1; l.Font = Enum.Font.Code\r\
            l.TextSize = 12; l.TextColor3 = Color3.fromRGB(0, 255, 80); l.TextTransparency = 0.55\r\
            l.Size = UDim2.fromOffset(12, 220); l.TextYAlignment = Enum.TextYAlignment.Top; l.Text = \"\"\r\
            l.Position = UDim2.fromScale((i - 0.5) / 10, math.random()); l.ZIndex = 2; l.Parent = vf\r\
            table.insert(self._matrix, l)\r\
        end\r\
\r\
        function self:SetModel(char)\r\
            for _, c in ipairs(self.World:GetChildren()) do c:Destroy() end\r\
            self.Model = nil\r\
...
\r\
        function self:_step(dt)\r\
            ...
            local t = tick()\r\
            -- world lighting -> viewport\r\
            self.VF.Ambient = GV.Color.fade(flags, \"World_Ambient\", t)\r\
            self.VF.LightColor = flags.World_Fullbright and Color3.new(1, 1, 1) or Color3.fromRGB(255, 255, 255)\r\
            -- chams (ESP o self-chams) sobre el clone\r\
            ...
            -- box overlay color (representativo)\r\
            self._box.Visible = flags.ESP_Box ~= false\r\
            self._boxStroke.Color = GV.Color.fade(flags, \"ESP_BoxColor\", t)\r\
            -- matrix rain\r\
            for i, l in ipairs(self._matrix) do\r\
                local y = (l.Position.Y.Scale + dt * (0.15 + (i % 3) * 0.08)) % 1.3 - 0.3\r\
                l.Position = UDim2.fromScale(l.Position.X.Scale, y)\r\
                if math.floor(t * 6 + i) % 3 == 0 then\r\
                    local s = {}; for k = 1, 9 do s[k] = string.char(48 + (i * 7 + k * 3) % 10) end\r\
