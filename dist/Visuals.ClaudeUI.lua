...
do local chunk = "return function(GV)\r\
    ...
    local ADD = { toggle = \"AddToggle\", slider = \"AddSlider\", dropdown = \"AddDropdown\",\r\
        colorpicker = \"AddColorPicker\", label = \"AddLabel\", textbox = \"AddInput\", button = \"AddButton\" }\r\
\r\
    function A.Tab(window, name) return window:AddTab(name) end\r\
    function A.Group(tab, name, side)\r\
        if side == \"Right\" then return tab:AddRightGroupbox(name) end\r\
        return tab:AddLeftGroupbox(name)\r\
    end\r\
    function A.Widget(group, kind, flag, opts, parent)\r\
        local m = ADD[kind]\r\
        if not m then warn(\"[claudeui] kind desconocido: \" .. tostring(kind)); return { flag = flag } end\r\
        local host = parent or group\r\
        -- nesting = dependencia; si el child no soporta este kind (ej AddInput/AddButton bajo toggle) -> al grupo\r\
        if type(host[m]) ~= \"function\" then host = group end\r\
        if type(host[m]) ~= \"function\" then warn(\"[claudeui] sin \" .. m); return { flag = flag } end\r\
        if kind == \"label\" then return host:AddLabel(opts.Text or \"\") end\r\
        if kind == \"button\" then return host:AddButton(opts.Text or \"Button\", opts.Callback or function() end) end\r\
        return host[m](host, flag, opts)\r\
    end\r\
    function A.Depend() end -- no-op: en ClaudeUI la dependencia se resuelve por nesting al crear\r\
    -- colorpicker pegado a un toggle\r\
